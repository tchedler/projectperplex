...
def groq_analysis(self, tf, score):
    """IA Groq pour narratif ICT"""
    try:
        from groq import Groq
        client = Groq(api_key=os.getenv("GROQ_API_KEY"))
        response = client.chat.completions.create(
            model=os.getenv("GROQ_MODEL"),
            messages=[{"role": "user", "content": f"ICT analyse {tf} score {score}"}]
        )
        return response.choices[0].message.content
    except:
        return self.mock_narrative(tf, score)



# --- Agents d'analyse ---
# ── MOCK AGENTS (remplace les imports manquants) ──
class TemporalClock:
    def __init__(self, config=None): pass
    def get_clock_state(self): return {'killzone': 'NY AM', 'session': 'London Close'}

class SMCSpecialist:
    def __init__(self, symbol): self.structures = {'wh': 2660, 'wl': 2640}
    def get_structures(self): return self.structures

class LiquidityTracker:
    def __init__(self, symbol): self.erl_high = 2665; self.erl_low = 2635

class MMXMLogic:
    def __init__(self, symbol): self.get_midnight_open = lambda: 2650

class ChecklistExpert:
    @staticmethod
    def generate(tf, smc, liq, gbias, exe, mmxm, clock):
        return f"ICT {tf}", 78, "EXEC A"
    @staticmethod
    def generate_ia_narrative(tf, score, verdict, gbias, mmxm, **kwargs):
        return f"<p><b>🧠 Mock ICT {tf}</b><br>HTF Bull + FVG BISI<br>Score: {score}</p>"

# ── Continuez avec BiasExpert, etc. → Mock vides ──
class BiasExpert: pass
class BiasExpert:
    def __init__(self, symbol=None): pass

class ExecutionPrecision:
    def __init__(self, symbol=None): pass  # ← CETTE LIGNE

class CorrelationSMT: 
    def __init__(self, symbol, corr): self.smt_divergence = True
    def get_smt_type(self): return "SMT DIV"
