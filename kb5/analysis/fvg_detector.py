...
"""
══════════════════════════════════════════════════════════════
Sentinel Pro KB5 — Détecteur FVG (Fair Value Gap) + Liquidity Void
══════════════════════════════════════════════════════════════
Responsabilités :
  - Détecter FVG Bullish (BISI) et Bearish (SIBI) sur tous les TF
  - Classifier chaque FVG : FRESH / MITIGATED / INVALID
  - Identifier les Liquidity Voids (grands FVG ou séquences de FVGs)
...
ATR_MIN_FACTOR = 0.15          # Réduit de 0.30 à 0.15 pour le Scalp
ATR_PERIOD     = 14            # période ATR standard

# ── Liquidity Void
LV_ATR_FACTOR        = 2.0    # FVG ≥ 2× ATR = Liquidity Void
LV_CONSECUTIVE       = 2      # ou 2+ FVGs consécutifs = Liquidity Void
LV_LOOKBACK          = 60     # fenêtre de scan
...
class FVGDetector(DetectorMixin):
    ...
    Liquidity Void :
        Extension extrême d'un FVG unique (≥ 2× ATR) ou séquence de
        2+ FVGs consécutifs, représentant une livraison de prix
        institutionnelle ultra-rapide sans découverte de prix réelle.
        Le prix est attiré à remplir ce "vide" lors du prochain move
        de rééquilibrage.
    """
    ...
    def __init__(self, data_store: DataStore, settings_integration=None):
        ...
        self._lv_cache: dict[str, dict[str, list]] = {}
        # _cache[pair][tf] = [list of FVG dicts]
        # _lv_cache[pair][tf] = [list of Liquidity Void dicts]
        logger.info("FVGDetector initialisé — FVG / NWOG / NDOG / Liquidity Void prêts")
    ...
    def scan_pair(self, pair: str) -> dict:
        ...
        for tf in Trading.TIMEFRAMES:
            ...
            # Détecter les Liquidity Voids
            atr_tf = self._calculate_atr(df)
            lv_list = self._detect_liquidity_voids(pair, tf, fvg_list, atr_tf)

            with self._lock:
                if pair not in self._lv_cache:
                    self._lv_cache[pair] = {}
                self._lv_cache[pair][tf] = lv_list

            logger.debug(
                f"FVG scan — {pair} {tf} | "
                f"total={len(fvg_list)} | "
                f"fresh={sum(1 for f in fvg_list if f['status'] == FVG_FRESH)} | "
                f"liq_void={len(lv_list)}"
            )
        ...
        with self._lock:
            ...
            for tf, new_fvg_list in results.items():
                ...
                # Pour le balisage intégral, on garde TOUS les FVGs (FRESH + MITIGATED + INVALID)
                self._cache[pair][tf] = final_list
    ...
    # ══════════════════════════════════════════════════════════
    # DÉTECTION LIQUIDITY VOIDS
    # ══════════════════════════════════════════════════════════

    def _detect_liquidity_voids(self, pair: str, tf: str,
                                 fvg_list: list, atr: float) -> list:
        """
        Détecte les Liquidity Voids dans une liste de FVG déjà détectés.

        Un Liquidity Void est identifié par :
          1. Un FVG unique dont la taille ≥ LV_ATR_FACTOR × ATR
          2. Ou LV_CONSECUTIVE FVGs consécutifs (même direction) dont au
             moins 1 est FRESH — indicateur de livraison algorithmique rapide.

        Le prix est statistiquement attiré à combler ces Voids lors
        d'un mouvement de rééquilibrage (Mean Reversion).

        Returns:
            liste de dicts Liquidity Void
        """
        lv_list  = []
        min_size = LV_ATR_FACTOR * atr

        # ── Règle 1 : FVG surdimensionné
        for fvg in fvg_list:
            if fvg.get("status") != FVG_FRESH:
                continue
            if fvg.get("size", 0) >= min_size:
                lv_list.append({
                    "id":         f"LV_{fvg['id']}",
                    "pair":       pair,
                    "tf":         tf,
                    ...
                    "size":       fvg["size"],
                    "atr_ratio":  round(fvg["size"] / atr, 2) if atr > 0 else 0,
                    "trigger":    "OVERSIZED_FVG",
                    "source_fvg": fvg["id"],
                    "formed_at":  fvg["formed_at"],
                    "status":     FVG_FRESH,
                })

        # ── Règle 2 : FVGs consécutifs même direction
        fresh_fvgs = [f for f in fvg_list if f.get("status") == FVG_FRESH]
        for direction in ("BULLISH", "BEARISH"):
            dir_fvgs = [f for f in fresh_fvgs if f["direction"] == direction]
            dir_fvgs = sorted(dir_fvgs, key=lambda f: f.get("candle_idx", 0))

            consecutive_count = 0
            group_start       = None

            for k, fvg in enumerate(dir_fvgs):
                if k == 0:
                    consecutive_count = 1
                    group_start = fvg
                else:
                    prev = dir_fvgs[k - 1]
                    # Consécutifs = index différence de 1 ou 2
                    if abs(fvg.get("candle_idx", 0) - prev.get("candle_idx", 0)) <= 2:
                        consecutive_count += 1
                    else:
                        consecutive_count = 1
                        group_start = fvg

                if consecutive_count >= LV_CONSECUTIVE and group_start:
                    # Zone englobée : du bottom du premier au top du dernier
                    group_bottom = group_start["bottom"]
                    group_top    = fvg["top"]
                    group_size   = abs(group_top - group_bottom)
                    lv_id        = f"LV_SEQ_{pair}_{tf}_{direction}_{group_start['candle_idx']}"

                    # Eviter les doublons
                    if not any(lv["id"] == lv_id for lv in lv_list):
                        lv_list.append({
                            "id":         lv_id,
                            "pair":       pair,
                            "tf":         tf,
                            "type":       "LIQUIDITY_VOID",
                            "direction":  direction,
                            "top":        round(group_top,    6),
                            "bottom":     round(group_bottom, 6),
                            "midpoint":   round((group_top + group_bottom) / 2, 6),
                            "size":       round(group_size, 6),
                            "atr_ratio":  round(group_size / atr, 2) if atr > 0 else 0,
                            "trigger":    "CONSECUTIVE_FVGS",
                            "source_fvg": group_start["id"],
                            "formed_at":  group_start["formed_at"],
                            "status":     FVG_FRESH,
                        })
    ...
    def get_liquidity_voids(self, pair: str, tf: str,
                             direction: Optional[str] = None) -> list:
        """
        Retourne les Liquidity Voids actifs pour une paire/TF.

        Un Liquidity Void est un grand FVG (≥ 2× ATR) ou une séquence
        de 2+ FVGs consécutifs dans la même direction. Ces zones attirent
        le prix lors du prochain mouvement de rééquilibrage institutionnel.
        ...
        Returns:
            liste de Liquidity Void dicts
        """
        with self._lock:
            lv_data = list(self._lv_cache.get(pair, {}).get(tf, []))

        if direction:
            lv_data = [lv for lv in lv_data if lv["direction"] == direction]

        return lv_data

    def get_fvg_count(self, pair: str, tf: str) -> dict:
        """
        Retourne un résumé des comptages par statut incluant les Liquidity Voids.
        Utilisé par scoring_engine pour évaluer la densité FVG.

        Returns:
            dict {fresh, invalid, liquidity_voids, total}
        """
        all_fvg = self.get_all_fvg(pair, tf)
        lv_list = self.get_liquidity_voids(pair, tf)
        return {
            "fresh":           sum(1 for f in all_fvg if f["status"] == FVG_FRESH),
            "invalid":         sum(1 for f in all_fvg if f["status"] == FVG_INVALID),
            "liquidity_voids": len(lv_list),
            "total":           len(all_fvg),
        }
