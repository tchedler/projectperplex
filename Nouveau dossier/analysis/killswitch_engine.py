...
class KillSwitchEngine:
    ...
    def evaluate(self, pair: str, direction: str = None) -> dict:
        ...
        def _skip_if_disabled(ks_key: str) -> dict:
            """Retourne un résultat passant si le KS est désactivé par l'utilisateur."""
            if ks_key.upper() in disabled_ks:
                logger.info(f"{ks_key.upper()} — Désactivé par l'utilisateur (bypass)")
                return {"active": False, "reason": "Désactivé par l'utilisateur", "disabled": True}
            return None

        # ── Évaluation dans l'ordre de priorité ─────────────
        ks_details["ks99"] = _skip_if_disabled("KS99") or self._check_ks99()
        ks_details["ks9"]  = _skip_if_disabled("KS9")  or self._check_ks9(pair, now)
        ks_details["ks5"]  = _skip_if_disabled("KS5")  or self._check_ks5(pair)
        ks_details["ks2"]  = _skip_if_disabled("KS2")  or self._check_ks2(pair)
        ks_details["ks1"]  = _skip_if_disabled("KS1")  or self._check_ks1(pair)
        ks_details["ks3"]  = _skip_if_disabled("KS3")  or self._check_ks3(pair, now)
        ks_details["ks6"]  = _skip_if_disabled("KS6")  or self._check_ks6(pair, direction)
        ks_details["ks7"]  = _skip_if_disabled("KS7")  or self._check_ks7(pair)
        ks_details["ks4"]  = _skip_if_disabled("KS4")  or self._check_ks4(now)
        ks_details["ks8"]  = _skip_if_disabled("KS8")  or self._check_ks8(pair)
