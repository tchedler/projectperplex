...
def _render_profiles(settings: SettingsManager) -> None:
    ...
    for i, pname in enumerate(profile_names):
        with cols[i]:
            ...
            if not is_active:
                if st.button(f"Appliquer", key=f"btn_profile_{pname}"):
...
def _render_pairs(settings: SettingsManager) -> None:
    ...
    # Aplatir le dictionnaire des paires en une seule liste
    all_available_pairs = []
    for pairs in AVAILABLE_PAIRS.values():
        all_available_pairs.extend(pairs)
    
    # Éviter les doublons et trier par ordre alphabétique
    all_available_pairs = sorted(list(set(all_available_pairs)))

    # Ne conserver comme sélection par défaut que les paires qui existent encore
    valid_current_pairs = [p for p in current_pairs if p in all_available_pairs]

    all_selected = st.multiselect(
        "Sélectionnez les paires à lancer :",
        options=all_available_pairs,
        default=valid_current_pairs,
        key="global_pairs_selector"
    )
    ...
    with col_s2:
        if st.button("💾 Sauvegarder les paires", key="btn_save_pairs"):
...
def _render_global_filters(settings: SettingsManager) -> None:
    ...
    # ── KillSwitches individuels ─────────────────────────
    st.markdown("---")
    st.markdown("### 🔴 KillSwitches individuels")
    st.caption(
        "Désactivez un KillSwitch spécifique si vous souhaitez forcer le trading "
        "malgré cette condition. **⚠️ Risque de pertes si mal calibré !**"
    )
    ...
    all_ks = [
        ...
        ("KS4",  "Hors Killzone ICT",         "Avertissement si hors session active"),
        ("KS5",  "Drawdown journalier max",   "Bloqué si DD du jour dépassé"),
        ("KS6",  "Contre-tendance HTF",       "Bloqué si biais HTF opposé"),
        ("KS7",  "Trop de positions",         "Bloqué si trop de trades ouverts"),
        ("KS8",  "Corrélation exposée",      "Avertissement corrélation"),
        ("KS9",  "Phase Accumulation",        "Bloqué si marché en range sans impulsion"),
    ]

    current_disabled = set(settings.get("disabled_ks", []))
    new_disabled = set()

    cols = st.columns(3)
    for i, (ks_id, label, desc) in enumerate(all_ks):
        with cols[i % 3]:
            disabled = st.toggle(
                f"🚫 {ks_id} — {label}",
                value=(ks_id in current_disabled),
                key=f"ks_toggle_{ks_id}",
                help=desc
            )
            if disabled:
                new_disabled.add(ks_id)

    if st.button("💾 Sauvegarder KillSwitches", key="btn_save_ks"):
        settings.set("disabled_ks", list(new_disabled))
        if new_disabled:
            st.warning(f"⚠️ {len(new_disabled)} KillSwitch(es) désactivé(s) : {', '.join(sorted(new_disabled))}")
        else:
            st.success("✅ Tous les KillSwitches sont actifs.")
...
def _render_ai_config(settings: SettingsManager) -> None:
    ...
    PROVIDERS = ["Gemini", "Grok (x.ai)", "OpenAI"]

    # Normaliser l'ancienne valeur "Grok" -> "Grok (x.ai)"
    current_provider = llm_conf["llm_provider"]
    if current_provider == "Grok":
        current_provider = "Grok (x.ai)"
