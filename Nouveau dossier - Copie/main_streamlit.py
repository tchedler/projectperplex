...
# ── Import Chart Builder (graphiques ICT annotés) ────────────
try:
...
# ── Import War Room (narratif IA Groq + Gemini) ──────────────
try:
    from interface.war_room import render_war_room
    WAR_ROOM_OK = True
except ImportError:
    WAR_ROOM_OK = False
...
def render_pair_summary(symbol: str, cache: dict):
    ...
    # ── Graphique Plotly annoté ───────────────────────────────
    if CHART_OK:
        # Sélecteur de TF pour le graphique
        tf_options = ["H1", "M15", "H4", "D1", "W1", "MN"]
        tf_sel = st.radio(
            "📈 Timeframe du graphique",
            options=tf_options,
            index=0,
            horizontal=True,
            key=f"tf_sel_{symbol}"
        )
        try:
            fig = build_chart_from_pkl(cache, tf=tf_sel, symbol=symbol)
            if fig is not None:
