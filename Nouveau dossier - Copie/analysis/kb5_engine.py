...
CONFLUENCE_IDM_BONUS      = CONFLUENCE_IDM          # Inducement (IDM) : stop hunt interne avant POI
CONFLUENCE_COT_BONUS      = CONFLUENCE_COT_ALIGNED  # COT aligné avec la direction du trade
...
class KB5Engine:
    ...
    def analyze(self, pair: str) -> dict:
        ...
        result = {
            ...
            # Structures actives (complètes pour dashboard)
    ...
    def _detect_confluences(self, pair: str, direction: str,
        ...
        # ── I. Inducement (IDM) — Stop Hunt interne avant POI ──
        # Un IDM frais avec un POI à portée = setup institutionnel haute probabilité.
        # Le marché a purgé les stops internes AVANT de toucher l'OB/FVG principal.
        if self._idm is not None:
            idm_dom = self._idm.get_dominant_idm(pair, direction)
            if idm_dom and idm_dom.get("status") == "FRESH" and idm_dom.get("poi_nearby"):
                sweeps_ago = idm_dom.get("candles_ago", 99)
                confluences.append({
                    "name":        "IDM_CONFIRM",
                    "bonus":       CONFLUENCE_IDM_BONUS,
                    "description": (
                        f"Inducement {direction} confirmé sur {idm_dom.get('tf', 'N/A')} "
                        f"— IDM prélevé il y a {sweeps_ago} bougie(s) "
                        f"@ {idm_dom.get('idm_level')} | POI à portée ✓"
                    ),
                    "idm_tf":      idm_dom.get("tf"),
                    "idm_level":   idm_dom.get("idm_level"),
                    "sweep_at":    idm_dom.get("swept_at"),
                })

        # ── J. COT + Saisonnalité (Biais Macro Institutionnel) ──
        # Si les Large Speculators (COT) sont alignés avec notre direction
        # ET que le mois est historiquement favorable → bonus fort.
        # Si le COT est opposé → pénalité : ne pas trader contre le flux institutionnel.
        if self._cot is not None:
            cot_result = self._cot.get_confluence_bonus(pair, direction)
            cot_bonus  = cot_result.get("bonus", 0)
            if cot_bonus != 0:
                cot_details = " | ".join(cot_result.get("details", []))
                confluences.append({
                    "name":        "COT_SEASONAL",
                    "bonus":       cot_bonus,
                    "description": (
                        f"Biais Macro institutionnel : {cot_details or 'COT/Saisonnalité'}"
                    ),
                    "macro_bias":  cot_result.get("macro_bias"),
                    "aligned":     cot_result.get("aligned"),
                    "cot_bonus":   cot_bonus,
                })
    ...
    def _get_active_structures(self, pair: str,
                                direction: str) -> dict:
        """
        Retourne l'intégralité des structures actives pour l'audit visuel.
        """
        all_fvg = []
        all_ob  = []
        all_bb  = []
        ...
        for tf in PYRAMID_ORDER:
            # FVG (Markup intégral)
            fvgs = self._fvg.get_all_fvg(pair, tf)
            if fvgs:
                for f in fvgs:
                    f["tf"] = tf
                    all_fvg.append(f)
            ...
            # OB (Markup intégral)
            obs = self._ob.get_all_ob(pair, tf)
            if obs:
                for o in obs:
                    o["tf"] = tf
                    all_ob.append(o)
            ...
            # Breakers (BULL + BEAR)
            bb_bull = self._ob.get_breakers(pair, tf, "BULLISH")
            if bb_bull:
                for b in bb_bull:
                    b["tf"] = tf
                    all_bb.append(b)
            
            bb_bear = self._ob.get_breakers(pair, tf, "BEARISH")
            if bb_bear:
                for b in bb_bear:
                    b["tf"] = tf
                    all_bb.append(b)
        ...
        # Récupération de la liquidité (BSL/SSL)
        liq_data = {}
        if self._liq:
            liq_snap = self._liq.get_snapshot(pair)
            liq_data = liq_snap.get("levels", {})
