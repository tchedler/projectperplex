...
Responsabilités :
  ...
  - Identifier Mitigation Blocks (OB ayant échoué à étendre la structure)
  - Identifier Rejection Blocks (longue mèche balayant de la liquidité)
...
ATR_PERIOD         = 14
ATR_IMPULSE_FACTOR = 1.1   # Réduit de 1.5 à 1.1 pour le Scalping Crypto
ATR_MIN_OB_SIZE    = 0.05  # Réduit de 0.10 à 0.05 pour capter les micro-OB
MAX_OB_LOOKBACK    = 60    # profondeur scan ICT (les vieux OB n'intéressent plus)

# ── Rejection Block
RJ_WICK_RATIO      = 0.60  # mèche ≥ 60% de la bougie totale
RJ_BODY_MAX_RATIO  = 0.35  # corps ≤ 35% de la bougie totale
RJ_LOOKBACK        = 50    # fenêtre de scan Rejection Blocks

# ── Mitigation Block
MB_LOOKBACK        = 60    # fenêtre de scan Mitigation Blocks
...
class OBDetector:
    ...
    Mitigation Block :
        OB qui a échoué à générer un Higher High (bullish) ou un Lower Low
        (bearish) après son retour. Ancienne zone de demande/offre devenue
        point de mitigation (rejet institutionnel).

    Rejection Block :
        Bougie à très longue mèche (≥ 60% de la range totale) dont le corps
        clôture dans la zone opposée. Indique un balayage de liquidité suivi
        d'un rejet violent — souvent coïncide avec un pool BSL/SSL.
    ...
    def __init__(self, data_store: DataStore, fvg_detector=None):
        ...
        #                              rejection_list, mitigation_list}
        logger.info("OBDetector initialisé — OB / Breaker / Mitigation / Rejection / BPR prêts")
    ...
    def scan_pair(self, pair: str) -> dict:
        ...
        for tf in Trading.TIMEFRAMES:
            ...
            mitigation_list  = self._detect_mitigation_blocks(pair, tf, df, atr)
            rejection_list   = self._detect_rejection_blocks(pair, tf, df, atr)
            ...
            logger.debug(
                f"OB scan — {pair} {tf} | "
                f"ob={len(ob_list)} | breakers={len(breakers)} | "
                f"mitigation={len(mitigation_list)} | rejection={len(rejection_list)} | "
                f"bpr={len(bpr_list)} | atr={round(atr, 6)}"
            )
        ...
        with self._lock:
            ...
            for tf, new_data in results.items():
                existing = self._cache[pair].get(tf, {
                    "ob_list": [], "breaker_list": [],
                    "mitigation_list": [], "rejection_list": [], "bpr_list": []
                })
                ...
                # Update cache
                self._cache[pair][tf] = {
                    "ob_list":         final_ob_list,
                    "breaker_list":    new_breakers,
                    "mitigation_list": new_data["mitigation_list"],
                    "rejection_list":  new_data["rejection_list"],
                    "bpr_list":        new_data["bpr_list"],
                }
    ...
    # ══════════════════════════════════════════════════════════
    # DÉTECTION MITIGATION BLOCKS
    # ══════════════════════════════════════════════════════════

    def _detect_mitigation_blocks(self, pair: str, tf: str,
                                   df: pd.DataFrame, atr: float) -> list:
        """
        Détecte les Mitigation Blocks.

        Un Mitigation Block est un OB (dernière bougie impulsive) qui a
        échoué à pousser le prix vers un nouveau High (bullish) ou nouveau Low
        (bearish). Le prix est retourné mitiger (effleurer) ce bloc sans
        casser la structure dans la direction opposée.

        Condition de détection :
          - Bougie impulsive (corps ≥ ATR_MIN_OB_SIZE × ATR)
          - Le mouvement suivant ÉCHOUE à casser le High (Bull) ou Low (Bear)
            de la bougie suivante sur N bougies (pas de nouveau HH/LL formé)
          - Le prix revient sur la zone du corps de la bougie originale

        Returns:
            liste de dicts Mitigation Block
        """
        mb_list  = []
        opens    = df["open"].values
        highs    = df["high"].values
        ...
        min_body = ATR_MIN_OB_SIZE * atr
        lookback = min(MB_LOOKBACK, len(df) - 5)

        for i in range(len(df) - lookback, len(df) - 4):
            if i < 0:
                continue
            ...
            # Fenêtre de vérification : 3 bougies après la bougie candidate
            look_end = min(i + 4, len(df))
            fwd_highs = highs[i+1:look_end]
            fwd_lows  = lows[i+1:look_end]

            if len(fwd_highs) < 2:
                continue

            if is_bull:
                # Bougie haussière qui échoue à créer un Higher High
                failed_extension = max(fwd_highs) <= highs[i]
                if not failed_extension:
                    continue
                # Vérif : le prix revient mitiger le corps de la bougie
                price_mitigates = min(fwd_lows) <= opens[i]  # revient sur l'open
                if not price_mitigates:
                    continue
                direction = "BULLISH"
                ob_type   = "BULL_MB"

            elif is_bear:
                # Bougie baissière qui échoue à créer un Lower Low
                failed_extension = min(fwd_lows) >= lows[i]
                if not failed_extension:
                    continue
                # Vérif : le prix revient mitiger le corps de la bougie
                price_mitigates = max(fwd_highs) >= opens[i]  # revient sur l'open
                if not price_mitigates:
                    continue
                direction = "BEARISH"
                ob_type   = "BEAR_MB"
            ...
            mb_list.append({
                "id":        f"MB_{ob_type}_{pair}_{tf}_{str(times[i])}",
                "pair":      pair,
                "tf":        tf,
                "type":      ob_type,
                "direction": direction,
    ...
    # ══════════════════════════════════════════════════════════
    # DÉTECTION REJECTION BLOCKS
    # ══════════════════════════════════════════════════════════

    def _detect_rejection_blocks(self, pair: str, tf: str,
                                   df: pd.DataFrame, atr: float) -> list:
        """
        Détecte les Rejection Blocks.

        Un Rejection Block est une bougie avec une très longue mèche (≥ 60%
        de la range totale) qui balaye de la liquidité (BSL ou SSL) et rejette
        violemment le prix dans la direction opposée.

        Condition de détection :
          - Range totale (High - Low) ≥ 0.5 × ATR (filtre petites bougies)
          - Mèche supérieure ≥ RJ_WICK_RATIO des range totale → Rejection Bearish
          - Mèche inférieure ≥ RJ_WICK_RATIO de la range totale → Rejection Bullish
          - Corps ≤ RJ_BODY_MAX_RATIO de la range totale

        Returns:
            liste de dicts Rejection Block
        """
        rj_list = []
        opens   = df["open"].values
        highs   = df["high"].values
        ...
        lookback = min(RJ_LOOKBACK, len(df) - 2)
        ...
        for i in range(len(df) - lookback, len(df) - 1):
            ...
            if total_range < 0.5 * atr:
                continue  # filtre : bougie trop petite
            ...
            if body_ratio > RJ_BODY_MAX_RATIO:
                continue  # corps trop gros : pas un Rejection Block

            if upper_ratio >= RJ_WICK_RATIO:
                # Longue mèche supérieure → Rejection Bearish (liquidity sweep BSL)
                rj_list.append({
                    "id":         f"RJ_BEAR_{pair}_{tf}_{str(times[i])}",
                    "pair":       pair,
                    "tf":         tf,
                    "type":       "BEAR_REJECTION",
                    "direction":  "BEARISH",
                    "top":        round(highs[i],  6),   # extrémité de la mèche
                    "bottom":     round(body_top,  6),   # sommet du corps
                    "wick_level": round(highs[i],  6),
                    "body_top":   round(body_top,  6),
                    "body_bot":   round(body_bot,  6),
                    "wick_ratio": round(upper_ratio, 3),
                    "body_ratio": round(body_ratio,  3),
                    "formed_at":  str(times[i]),
                    "status":     OB_VALID,
                })

            elif lower_ratio >= RJ_WICK_RATIO:
                # Longue mèche inférieure → Rejection Bullish (liquidity sweep SSL)
                rj_list.append({
                    "id":         f"RJ_BULL_{pair}_{tf}_{str(times[i])}",
                    "pair":       pair,
                    "tf":         tf,
                    "type":       "BULL_REJECTION",
                    "direction":  "BULLISH",
                    "top":        round(body_bot,  6),   # bas du corps
                    "bottom":     round(lows[i],   6),   # extrémité de la mèche
                    "wick_level": round(lows[i],   6),
                    "body_top":   round(body_top,  6),
                    "body_bot":   round(body_bot,  6),
                    "wick_ratio": round(lower_ratio, 3),
                    "body_ratio": round(body_ratio,  3),
                    "formed_at":  str(times[i]),
                    "status":     OB_VALID,
                })
    ...
    def get_all_ob(self, pair: str, tf: str,
                   status: str = None) -> list:
        """
        Retourne tous les OB d'une paire/TF (Markup intégral).
        """
        with self._lock:
            data = self._cache.get(pair, {}).get(tf, {})
        ...
        ob_list = data.get("ob_list", [])
        if status:
            ob_list = [o for o in ob_list if o["status"] == status]
        return ob_list
    ...
    def get_mitigation_blocks(self, pair: str, tf: str,
                               direction: str = None) -> list:
        """
        Retourne les Mitigation Blocks détectés.

        Un Mitigation Block est un OB ayant échoué à étendre la structure dans
        sa direction originale. Le prix est revenu mitiger cette zone avant de
        continuer dans la direction opposée.

        Returns:
            liste Mitigation Block dicts
        """
        with self._lock:
            data = self._cache.get(pair, {}).get(tf, {})

        mb_list = data.get("mitigation_list", [])
        if direction:
            mb_list = [m for m in mb_list if m["direction"] == direction]
        return mb_list

    def get_rejection_blocks(self, pair: str, tf: str,
                              direction: str = None) -> list:
        """
        Retourne les Rejection Blocks détectés.

        Un Rejection Block est signalé par une longue mèche (≥ 60% de la range
        totale) balayant une pool de liquidité et rejetant le prix violemment.

        Returns:
            liste Rejection Block dicts
        """
        with self._lock:
            data = self._cache.get(pair, {}).get(tf, {})

        rj_list = data.get("rejection_list", [])
        if direction:
            rj_list = [r for r in rj_list if r["direction"] == direction]
        return rj_list
    ...
    def get_ob_count(self, pair: str, tf: str) -> dict:
        ...
        Returns:
            dict {valid, tested, broken, breakers, mitigation, rejection, bpr, total}
        """
        with self._lock:
            data = self._cache.get(pair, {}).get(tf, {})
        ...
        ob_list    = data.get("ob_list",         [])
        breakers   = data.get("breaker_list",    [])
        mitigation = data.get("mitigation_list", [])
        rejection  = data.get("rejection_list",  [])
    ...
    def get_snapshot(self, pair: str) -> dict:
        ...
        for tf, data in pair_data.items():
            ...
            snapshot[tf] = {
                ...
                "mitigation": len(data.get("mitigation_list", [])),
                "rejection":  len(data.get("rejection_list",  [])),
