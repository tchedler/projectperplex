...
class Supervisor:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            if threading.current_thread() is threading.main_thread():
                ...
                logger.warning("Supervisor lancé en thread secondaire — signaux OS désactivés.")
    ...
    def _process_pair(self, pair: str) -> None:
        ...
        Pipeline complet pour une paire.
          ...
          1. Analyser KB5 complètement (pour le dashboard)
          2. Évaluer KS (pré-check rapide)
          3. Évaluer scoring (analyse complète)
          4. Si EXECUTE → allocate → shield → order
          5. Si WATCH   → log + dashboard
        """
        
        # ── Analyse KB5 COMPLÈTE (pour dashboard) ────────────
        # Faire l'analyse KB5 AVANT le check KS pour que le dashboard
        # affiche les données d'analyse même si la paire est bloquée
        ...
        if ks_result["verdict"] == "BLOCKED":
            ...
            # ── Pont Streamlit: pousser statut + données KB5 ───────────
            blocked_output = {
                "pair": pair,
                "score": 0,
                "grade": "F",
                "verdict": "BLOCKED",
                "direction": "NEUTRAL",
                "reason": ks_result['blocked_by'],
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
            # Mettre à jour DataStore pour que MarketStateCache rafraîchisse le timestamp
            self._ds.set_analysis(pair, "scoring", blocked_output)
            # Les données KB5 sont déjà écrites ci-dessus par kb5_result = self._kb5.analyze(pair)
            return
