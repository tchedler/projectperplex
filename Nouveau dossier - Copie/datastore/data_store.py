...
class DataStore:
    ...
    def __init__(
        ...
        # ── Pont Frontend (Streamlit MarketStateCache) ────────
        # Injecté par main.py pour que les analyses soient
        # poussées en temps réel vers market_state.pkl
    ...
    def set_candles(self, pair: str, timeframe: str, df):
        ...
        with self._lock:
            ...
            # ── Pont Streamlit : pousser les bougies H1 vers MarketStateCache ─
            # Le dashboard utilise H1 par défaut pour l'affichage du graphique principal.
            if self._market_cache is not None and timeframe == "H1":
                try:
                    existing = self._market_cache.get(pair, {}) or {}
                    
                    # Convertir DataFrame OHLCV + infos manquantes en liste de dictionnaires pour le main chart
                    candles_list = []
                    if not df.empty:
                         # Ensure the index is available as 'time' or we use the index if it's datetime
                         df_copy = df.copy()
                         if df_copy.index.name == 'time' or type(df_copy.index) == pd.DatetimeIndex:
                    ...
                    existing["candles"] = candles_list
                    self._market_cache.update({pair: existing})
                except Exception as _e:
                    logger.debug(f"DataStore — market_cache candles update ignoré : {_e}")
    ...
    def set_analysis(self, pair: str, timeframe: str, result: dict):
        ...
        with self._lock:
            ...
            # ── Pont Streamlit : pousser vers MarketStateCache ─
            # Permet au dashboard de lire les données en temps réel
            # sans poll JSON — écriture PKL atomique via le cache.
            if self._market_cache is not None:
                try:
                    existing = self._market_cache.get(pair, {}) or {}
                    # Mapper timeframe → clé attendue par dashboard.py
                    _key_map = {
                    ...
                    cache_key = _key_map.get(timeframe, timeframe + "_result")
                    existing[cache_key]     = result
                    existing["last_update"] = datetime.utcnow().isoformat()
                    self._market_cache.update({
                        pair:                 existing,
                        "bot_status":         "Actif",
                        "last_global_update": datetime.utcnow().isoformat(),
                    })
                except Exception as _e:
                    logger.debug(f"DataStore — market_cache update ignoré : {_e}")
    ...
    def set_ks_state(self, ks_id: int, active: bool, reason: str = ""):
        ...
        with self._lock:
            ...
            if self._market_cache is not None:
                try:
                    # On construit le statut dynamiquement
                    active_ks = [str(k) for k, s in self._ks_state.items() if s.get("active")]
                    status = "Actif"
                    if active_ks:
                        status = f"Bloqué (KS: {', '.join(active_ks)})"
                    ...
                    logger.debug(f"DataStore — market_cache KS ignoré : {_e}")
    ...
    def set_cb_state(
        ...
        with self._lock:
            ...
            if self._market_cache is not None:
                try:
                    bot_status = "Actif" if level == 0 else f"Bloqué (CB Niveau {level})"
                    self._market_cache.update({
                        "bot_status": bot_status,
                        "last_global_update": datetime.utcnow().isoformat(),
                    })
                except Exception as _e:
                    logger.debug(f"DataStore — market_cache CB ignoré : {_e}")
    ...
    def set_positions_cache(
        ...
        with self._lock:
            ...
            if self._market_cache is not None:
                try:
                    self._market_cache.update({"open_positions": positions})
                except Exception as _e:
                    logger.debug(f"DataStore — market_cache positions ignoré : {_e}")
    ...
    def set_equity(self, equity: float) -> None:
        ...
        if self._market_cache is not None:
            try:
                self._market_cache.update({"equity": float(equity)})
            except Exception as _e:
                logger.debug(f"DataStore — market_cache equity ignoré : {_e}")
    ...
    def save_to_disk(self):
        ...
        try:
            with self._lock:
                ...
                state = {
                    "analysis": clean_for_json(self._analysis),
                    "metadata": clean_for_json(self._metadata),
                    "ks_state": clean_for_json(self._ks_state),
                    "cb_state": clean_for_json(self._cb_state),
                }
...
# ══════════════════════════════════════════════════════════════
# UTILITAIRE : ENCODER JSON + NETTOYEUR DONNÉES
# ══════════════════════════════════════════════════════════════

def clean_for_json(obj):
    """
    Nettoie récursivement un objet pour le rendre JSON-sérialisable.
    ...
    # Dictionnaire — nettoyer récursivement
    if isinstance(obj, dict):
        return {k: clean_for_json(v) for k, v in obj.items()}
    
    # Liste — nettoyer récursivement
...
class DataStoreEncoder(json.JSONEncoder):
    """Permet de sérialiser les objets datetime et types NumPy/Pandas en JSON."""
